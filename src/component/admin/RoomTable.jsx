...
const RoomTable = () => {
    ...
    const [newRoom, setNewRoom] = useState({ roomNumber: "", status: "", beds: "", image: "" }); // ข้อมูลห้องใหม่

    // ฟังก์ชันเพิ่มห้อง
    const addRoom = () => {
        if (newRoom.roomNumber && newRoom.status && newRoom.beds && newRoom.image) {
            const newRoomData = { ...newRoom, id: rooms.length + 1 };
            setRooms([...rooms, newRoomData]);
            setNewRoom({ roomNumber: "", status: "", beds: "", image: "" }); // รีเซ็ตฟอร์ม
        } else {
